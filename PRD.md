...
基於程式碼簡化分析（code-simplifier）與架構師評估（Gemini），建議以下重構方向：

#### 優先級 1：提取共用模組（低風險、高效益）

| 項目 | 建議 | 影響檔案 |
|------|------|----------|
| 輸出格式化 | 建立 `src/lib/formatter.ts` 統一格式化邏輯 | list.ts, range.ts, month.ts 等 |
| 輸出服務 | 建立 `src/lib/output.ts` 封裝 console 輸出 | 所有 command 檔案 |
| 測試工具 | 提取 mock 到 `tests/helpers/` | 所有測試檔案 |

#### 優先級 2：服務層重構（中風險、高效益）

| 項目 | 建議 | 說明 |
|------|------|------|
| Repository 模式 | 建立 `HolidayRepository` | 封裝快取與 API 邏輯 |
| 拆分 HolidayService | 分離為 Calculator + Filter | 遵循單一職責原則 |
| 統一日期處理 | 強化 `date-parser.ts` | 考慮引入 `date-fns` 或 `dayjs` |

#### 優先級 3：程式碼品質（低風險、中效益）

| 項目 | 建議 |
|------|------|
| 類型強化 | 減少 `any` 使用，加強類型定義 |
| 錯誤處理 | 建立統一錯誤處理機制 |
| 常數集中 | 將散落的配置集中到 `src/constants/` |
| 命令工廠 | 考慮命令工廠模式減少重複程式碼 |

#### 重構原則

1. **漸進式重構**：每次只改一個模組，確保測試通過
2. **測試先行**：利用 93%+ 覆蓋率確保重構不破壞功能
3. **向後相容**：API 介面維持穩定，內部實作漸進改善
