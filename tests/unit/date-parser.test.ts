...
describe('date-parser', () => {
  describe('parseDate', () => {
    describe('standard formats', () => {
      it('should parse YYYY-MM-DD format', () => {
        const result = parseDate('2026-10-10');
        expect(result.year).toBe(2026);
        expect(result.month).toBe(10);
        expect(result.day).toBe(10);
        expect(result.normalized).toBe('20261010');
        expect(result.iso).toBe('2026-10-10');
      });

      it('should parse YYYYMMDD format', () => {
        const result = parseDate('20261010');
        expect(result.year).toBe(2026);
        expect(result.month).toBe(10);
        expect(result.day).toBe(10);
        expect(result.normalized).toBe('20261010');
      ...
      it('should handle single digit month and day', () => {
        ...
        expect(result.normalized).toBe('20260105');
      });
    });
    ...
    describe('validation', () => {
      ...
      it('should throw DateParseError for invalid date', () => {
        expect(() => parseDate('2026-02-30')).toThrow(DateParseError);
      });
      ...
      it('should throw DateParseError for invalid month', () => {
        expect(() => parseDate('2026-13-01')).toThrow(DateParseError);
        expect(() => parseDate('2026-00-01')).toThrow(DateParseError);
      });

      it('should throw DateParseError for invalid day', () => {
        expect(() => parseDate('2026-01-32')).toThrow(DateParseError);
        expect(() => parseDate('2026-01-00')).toThrow(DateParseError);
      });
    });
  });
  ...
  describe('parseRelativeDate', () => {
    beforeEach(() => {
      // Mock current date to 2026-01-05 (Monday)
      vi.useFakeTimers();
    ...
    it('should parse "yesterday"', () => {
      ...
      expect(result.iso).toBe('2026-01-04');
    });

    it('should parse "next monday"', () => {
      const result = parseRelativeDate('next monday');
      expect(result.iso).toBe('2026-01-12'); // 2026-01-05 is Monday, next Monday is 2026-01-12
    });

    it('should parse "next friday"', () => {
      const result = parseRelativeDate('next friday');
      expect(result.iso).toBe('2026-01-09');
    });
    ...
    it('should parse "2w" (2 weeks)', () => {
      ...
      expect(result.iso).toBe('2026-01-19');
    });

    it('should parse "1m" (1 month)', () => {
      const result = parseRelativeDate('1m');
      expect(result.iso).toBe('2026-02-05');
    });

    it('should return null for non-relative date', () => {
      const result = parseRelativeDate('2026-01-01');
  ...
  describe('isValidDate', () => {
    ...
    it('should return false for invalid month', () => {
      ...
      expect(isValidDate(2026, 0, 10)).toBe(false);
    });
  ...
  describe('getWeekdayName', () => {
    it('should return correct weekday name', () => {
      ...
      expect(getWeekdayName(2026, 1, 5)).toBe('一');
      // 2026-01-06 is Tuesday
      expect(getWeekdayName(2026, 1, 6)).toBe('二');
      // 2026-10-10 is Saturday
      expect(getWeekdayName(2026, 10, 10)).toBe('六');
    });
  });
  ...
  describe('getCurrentDate', () => {
    ...
    it('should return current date as ParsedDate', () => {
      ...
      expect(result.normalized).toBe('20260615');
      expect(result.iso).toBe('2026-06-15');
    });
  });
  ...
  describe('addDays', () => {
    it('should add positive days', () => {
      const result = addDays({ year: 2026, month: 1, day: 15, normalized: '20260115', iso: '2026-01-15' }, 10);
      expect(result.year).toBe(2026);
      expect(result.month).toBe(1);
      expect(result.day).toBe(25);
      expect(result.iso).toBe('2026-01-25');
    });

    it('should handle month overflow', () => {
      const result = addDays({ year: 2026, month: 1, day: 25, normalized: '20260125', iso: '2026-01-25' }, 10);
      expect(result.year).toBe(2026);
      expect(result.month).toBe(2);
      expect(result.day).toBe(4);
      expect(result.iso).toBe('2026-02-04');
    });
  ...
  describe('addMonths', () => {
    it('should add positive months', () => {
      const result = addMonths({ year: 2026, month: 1, day: 15, normalized: '20260115', iso: '2026-01-15' }, 3);
      expect(result.year).toBe(2026);
    ...
    it('should handle day overflow (e.g., Jan 31 + 1 month)', () => {
      const result = addMonths({ year: 2026, month: 1, day: 31, normalized: '20260131', iso: '2026-01-31' }, 1);
    ...
    it('should handle negative months', () => {
      const result = addMonths({ year: 2026, month: 3, day: 15, normalized: '20260315', iso: '2026-03-15' }, -2);
      expect(result.year).toBe(2026);
      expect(result.month).toBe(1);
      expect(result.day).toBe(15);
    });
  ...
  describe('dateToNormalized', () => {
    it('should convert Date to normalized string', () => {
      ...
      expect(dateToNormalized(date)).toBe('20260615');
    });

    it('should pad single digit month and day', () => {
      const date = new Date('2026-01-05');
      expect(dateToNormalized(date)).toBe('20260105');
    });
  });

  describe('parsedDateToDate', () => {
    it('should convert ParsedDate to Date', () => {
      const parsed = { year: 2026, month: 6, day: 15, normalized: '20260615', iso: '2026-06-15' };
  ...
  describe('daysBetween', () => {
    it('should calculate days between two dates', () => {
      const start = { year: 2026, month: 1, day: 1, normalized: '20260101', iso: '2026-01-01' };
      const end = { year: 2026, month: 1, day: 10, normalized: '20260110', iso: '2026-01-10' };
      expect(daysBetween(start, end)).toBe(10);
    });

    it('should return 1 for same day', () => {
      const date = { year: 2026, month: 1, day: 15, normalized: '20260115', iso: '2026-01-15' };
      expect(daysBetween(date, date)).toBe(1);
    });

    it('should handle cross-month dates', () => {
      const start = { year: 2026, month: 1, day: 25, normalized: '20260125', iso: '2026-01-25' };
      const end = { year: 2026, month: 2, day: 5, normalized: '20260205', iso: '2026-02-05' };
  ...
  describe('getYearsInRange', () => {
    it('should return years in range', () => {
      const start = { year: 2026, month: 1, day: 1, normalized: '20260101', iso: '2026-01-01' };
      const end = { year: 2026, month: 12, day: 31, normalized: '20261231', iso: '2026-12-31' };
      expect(getYearsInRange(start, end)).toEqual([2026]);
    });
    ...
    it('should expand search range when expandMonths is set', () => {
      const start = { year: 2026, month: 2, day: 1, normalized: '20260201', iso: '2026-02-01' };
      const end = { year: 2026, month: 2, day: 28, normalized: '20260228', iso: '2026-02-28' };
      // With expandMonths=1, should include Jan-Mar
      expect(getYearsInRange(start, end, 1)).toEqual([2026]);
    });

    it('should expand into previous year', () => {
      const start = { year: 2026, month: 1, day: 15, normalized: '20260115', iso: '2026-01-15' };
      const end = { year: 2026, month: 1, day: 20, normalized: '20260120', iso: '2026-01-20' };
      // With expandMonths=1, search starts from Dec 2025
      expect(getYearsInRange(start, end, 1)).toEqual([2025, 2026]);
    });
  });
});
